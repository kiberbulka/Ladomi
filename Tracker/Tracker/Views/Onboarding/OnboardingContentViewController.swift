...
final class OnboardingContentViewController: UIViewController {
    ...
    lazy var button: UIButton = {
        ...
        button.titleLabel?.font = .trackerMedium(16)
    ...
    private lazy var mockupContentView: UIView = {
        configuration.isAnalyticsFocused ? makeAnalyticsMockup() : makeTrackerMockup()
    }()
    ...
    private func makeTrackerMockup() -> UIView {
        let container = UIView()

        let title = makeMockupLabel(text: NSLocalizedString("onboarding.mockup.today", comment: "Сегодня"), font: .trackerBold(24), color: .ypBlack)
        ...
        let habitCard = makeMiniTrackerCard(emoji: "💧", title: NSLocalizedString("onboarding.mockup.habit", comment: "Привычка"), subtitle: "5 \(NSLocalizedString("tracker.days", comment: ""))", color: configuration.accentColor, isDone: true)
        let eventCard = makeMiniTrackerCard(emoji: "📚", title: NSLocalizedString("onboarding.mockup.event", comment: "Событие"), subtitle: NSLocalizedString("onboarding.mockup.eventDate", comment: "Дата события"), color: configuration.secondaryColor, isDone: false)
        let chipStack = makeHorizontalStack([
            ...
            makeCapsuleLabel(text: NSLocalizedString("uncompletedTrackers", comment: ""), backgroundColor: .ypGray, textColor: .ypLightGray)
        ], spacing: 8)
    ...
    private func makeAnalyticsMockup() -> UIView {
        ...
        let title = makeMockupLabel(text: NSLocalizedString("analytics.title", comment: ""), font: .trackerBold(24), color: .ypBlack)
        let progressRing = makeProgressRingView()
        let percentLabel = makeMockupLabel(text: "82%", font: .trackerBold(28), color: .ypBlack)
        let periodCard = makeStatisticCard(value: "7", titleKey: "currentStreak", tintColor: configuration.accentColor)
        let completedCard = makeStatisticCard(value: "24", titleKey: "numberOfCompletedTrackers", tintColor: configuration.secondaryColor)
    ...
    private func makeSearchView() -> UIView {
        ...
        let label = makeMockupLabel(text: NSLocalizedString("searchBar", comment: ""), font: .trackerRegular(15), color: .ypLightGray)
    ...
    private func makeMiniTrackerCard(emoji: String, title: String, subtitle: String, color: UIColor, isDone: Bool) -> UIView {
        let card = UIView()
        ...
        emojiLabel.font = .trackerMedium(22)
        emojiLabel.textAlignment = .center
        emojiLabel.backgroundColor = UIColor.white.withAlphaComponent(0.22)
        emojiLabel.layer.cornerRadius = 20
        emojiLabel.layer.masksToBounds = true

        let titleLabel = makeMockupLabel(text: title, font: .trackerBold(17), color: .ypWhite)
        titleLabel.numberOfLines = 2

        let subtitleLabel = makeMockupLabel(text: subtitle, font: .trackerMedium(12), color: UIColor.white.withAlphaComponent(0.85))
    ...
    private func makeStatisticCard(value: String, titleKey: String, tintColor: UIColor) -> UIView {
        ...
        let valueLabel = makeMockupLabel(text: value, font: .trackerBold(18), color: .ypBlack)
        let titleLabel = makeMockupLabel(text: NSLocalizedString(titleKey, comment: ""), font: .trackerMedium(12), color: .ypLightGray)
    ...
    private func makeAdviceCard() -> UIView {
        ...
        let titleLabel = makeMockupLabel(text: NSLocalizedString("analytics.advice.title", comment: ""), font: .trackerBold(17), color: .ypWhite)
        let detailLabel = makeMockupLabel(text: NSLocalizedString("onboarding.mockup.advice", comment: "Совет аналитики"), font: .trackerMedium(13), color: UIColor.white.withAlphaComponent(0.72))
