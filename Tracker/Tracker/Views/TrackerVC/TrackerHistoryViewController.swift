...
final class TrackerHistoryViewController: UIViewController {
    ...
    private lazy var titleLabel: UILabel = {
        let label = UILabel()
        label.text = NSLocalizedString("trackerHistory.title", comment: "Tracker history title")
        label.font = .trackerBold(24)
    ...
    private lazy var emojiLabel: UILabel = {
        let label = UILabel()
        label.text = tracker.emoji
        label.font = .trackerRegular(24)
        label.textAlignment = .center
    ...
    private lazy var summaryStackView: UIStackView = {
        let stackView = UIStackView(arrangedSubviews: [
            ...
            makeSummaryView(
                ...
                title: NSLocalizedString("historyCurrentStreak", comment: "Current tracker streak")
            )
        ])
    ...
    private lazy var legendStackView: UIStackView = {
        let stackView = UIStackView(arrangedSubviews: [
            makeLegendItem(color: tracker.color, title: NSLocalizedString("historyLegendCompleted", comment: "Completed legend")),
    ...
    private var historyEndDate: Date {
        calendar.startOfDay(for: tracker.archivedDate ?? Date())
    }

    private var missedDatesCount: Int {
        let today = calendar.startOfDay(for: Date())
        return historicalActiveDates().filter { date in
            let shouldCountMissed = tracker.isArchived ? date <= historyEndDate : date < today
    ...
    private var currentStreak: Int {
        ...
        for date in historicalActiveDates().reversed() where tracker.isArchived || date < today || completedDates.contains(today) {
    ...
    init(tracker: Tracker, records: [TrackerRecord]) {
        ...
        self.displayedMonth = calendar.startOfDay(for: tracker.archivedDate ?? Date())
    ...
    private func setupUI() {
        [titleLabel, closeButton, trackerCardView, summaryStackView, monthHeaderStackView, weekdaysStackView, collectionView, legendStackView].forEach {
        ...
        NSLayoutConstraint.activate([
            ...
            trackerCardView.topAnchor.constraint(equalTo: titleLabel.bottomAnchor, constant: 24),
            trackerCardView.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 16),
            trackerCardView.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -16),
            trackerCardView.heightAnchor.constraint(equalToConstant: 108),

            emojiLabel.leadingAnchor.constraint(equalTo: trackerCardView.leadingAnchor, constant: 16),
            emojiLabel.centerYAnchor.constraint(equalTo: trackerCardView.centerYAnchor),
            emojiLabel.widthAnchor.constraint(equalToConstant: 40),
            emojiLabel.heightAnchor.constraint(equalToConstant: 40),

            trackerNameLabel.leadingAnchor.constraint(equalTo: emojiLabel.trailingAnchor, constant: 12),
            trackerNameLabel.trailingAnchor.constraint(equalTo: trackerCardView.trailingAnchor, constant: -16),
            trackerNameLabel.centerYAnchor.constraint(equalTo: trackerCardView.centerYAnchor),

            summaryStackView.topAnchor.constraint(equalTo: trackerCardView.bottomAnchor, constant: 16),
    ...
    private func makeSummaryView(value: String, title: String) -> UIView {
        ...
        valueLabel.font = .trackerBold(26)
        valueLabel.textColor = .ypBlack
        valueLabel.textAlignment = .center

        let titleLabel = UILabel()
        titleLabel.text = title
        titleLabel.font = .trackerMedium(12)
    ...
    private func state(for date: Date) -> DayState {
        ...
        guard isTrackerActive(on: date) else {
            return .empty
        }

        if tracker.isArchived && date > historyEndDate {
            return .empty
        }
    ...
    private func historyStartDate() -> Date? {
        ...
        let eventDate = tracker.eventDate.map { calendar.startOfDay(for: $0) }
    ...
    private func isTrackerActive(on date: Date) -> Bool {
        if tracker.isHabit {
        ...
        let eventDate = calendar.startOfDay(for: tracker.eventDate ?? historyEndDate)
...
extension TrackerHistoryViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.configure(with: dayItems[indexPath.item], trackerColor: tracker.color)
        return cell
    }
}

extension TrackerHistoryViewController: UICollectionViewDelegateFlowLayout {
    func collectionView(
...
private final class HistoryDayCell: UICollectionViewCell {
    ...
    func configure(with item: TrackerHistoryViewController.DayItem, trackerColor: UIColor) {
        ...
        case .completed:
            dayLabel.backgroundColor = trackerColor
