...
final class DayCalendarViewController: UIViewController {
    ...
    private final class MoodStore {
        private let storageKey = "tracker.dayMoodByDate"
    ...
    private var trackerByID: [UUID: Tracker] = [:]
    private var recordsByDate: [Date: [TrackerRecord]] = [:]
    ...
    private lazy var moodStackView: UIStackView = {
        let buttons = DayMood.allCases.map { mood -> UIButton in
            ...
            button.titleLabel?.font = .trackerRegular(22)
    ...
    private func reloadCalendar() {
        ...
        trackerByID = Dictionary(uniqueKeysWithValues: trackers.map { ($0.id, $0) })
        recordsByDate = Dictionary(grouping: trackerRecordStore.fetch()) { record in
    ...
    private func activity(for date: Date) -> DayActivity {
        ...
        records.forEach { record in
            guard let tracker = trackerByID[record.trackerID] else {
                return
            }
        ...
        let trackerColors = records.compactMap { record in
            trackerByID[record.trackerID]?.color
        }

        return DayActivity(habitCount: habitCount, eventCount: eventCount, trackerColors: trackerColors)
    }
...
private final class DayCalendarCell: UICollectionViewCell {
    ...
    fileprivate func configure(
        ...
        configureDots(with: activity.trackerColors)
