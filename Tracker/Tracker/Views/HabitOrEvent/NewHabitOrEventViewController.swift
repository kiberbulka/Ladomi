...
final class NewHabitOrEventViewController: UIViewController, CategorySelectionDelegate {
    private enum TrackerSettingsRow: Equatable {
        case category
        case schedule
        case eventDate
        case reminder
    ...
    private var settingsRows: [TrackerSettingsRow] {
    ...
    private lazy var trackerNameTF: UITextField = {
        ...
        textField.font = .trackerMedium(17)
        ...
        textField.addTarget(self, action: #selector(trackerNameDidChange), for: .editingChanged)
        return textField
    }()
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            trackerNameTF.heightAnchor.constraint(equalToConstant: 64),
            ...
            characterLimitLabel.topAnchor.constraint(equalTo: trackerNameTF.bottomAnchor, constant: 4),
            ...
            trackerNameTF.widthAnchor.constraint(equalTo: contentView.widthAnchor, constant: -40)
    ...
    private func makeTypeChip(title: String, systemImageName: String, isSelected: Bool, selectsHabit: Bool) -> UIButton {
        ...
        button.isEnabled = !isEditingTracker
    ...
    private func updatePreviewCard() {
        ...
        let title = (trackerNameTF.text ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    @objc private func createButtonDidTap() {
        ...
        } else if presentingViewController is CreateTrackerViewController {
    ...
    private func makeTracker() -> Tracker {
        ...
        let id = isEditingTracker ? trackerToEdit?.id ?? UUID() : UUID()
        let today = Date()
        ...
        return Tracker(
            ...
            createdDate: trackerToEdit?.createdDate ?? today,
            archivedDate: trackerToEdit?.archivedDate,
            isArchived: trackerToEdit?.isArchived ?? false
        )
    }
    ...
    private func eventDate(for fallbackDate: Date) -> Date? {
        ...
        return selectedEventDate ?? trackerToEdit?.eventDate ?? fallbackDate
    }

    private func prepareInitialEventDate() {
        guard !isHabit else { return }

        selectedEventDate = normalizedEventDate(selectedEventDate ?? trackerToEdit?.eventDate)
    }
    ...
    private func configureSettingsIcon(for cell: UITableViewCell, row: TrackerSettingsRow) {
    ...
    private func editTracker() {
        ...
        selectedReminderTime = tracker.reminderTime
        selectedEventDate = tracker.isHabit ? nil : normalizedEventDate(tracker.eventDate)
        isHabit = tracker.isHabit
        updateTypeSegmentView()
        let text = NSLocalizedString("editHabit", comment: "")
        newHabitLabel.text = text

        countDaysLabel.isHidden = !tracker.isHabit
...
extension NewHabitOrEventViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.font = .trackerBold(19)
        cell.detailTextLabel?.font = .trackerMedium(15)
