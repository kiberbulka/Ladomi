...
struct TodayProgressProvider: TimelineProvider {
    ...
    private func loadSnapshot() -> TrackerTodayWidgetSnapshot {
        guard let data = TrackerWidgetShared.storage.data(forKey: TrackerWidgetShared.todaySnapshotKey),
              let snapshot = try? JSONDecoder().decode(TrackerTodayWidgetSnapshot.self, from: data),
    ...
    var body: some View {
        HStack(spacing: 12) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(snapshot.hasTrackers ? "\(snapshot.completedCount)/\(snapshot.totalCount)" : "0/0")
                    .font(.system(size: 32, weight: .bold))

                ProgressView(value: snapshot.progress)
                    .tint(.blue)

                Text(snapshot.completedCount == snapshot.totalCount && snapshot.hasTrackers ? "День закрыт" : "План на день")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
...
private struct CircularTodayWidget: View {
    ...
    var body: some View {
        Gauge(value: snapshot.progress) {
            ...
            Text(snapshot.hasTrackers ? "\(snapshot.completedCount)/\(snapshot.totalCount)" : "0")
...
private struct RectangularTodayWidget: View {
    ...
    private var titleText: String {
        snapshot.hasTrackers ? "\(snapshot.completedCount) из \(snapshot.totalCount)" : "Нет привычек"
    }

    private var subtitleText: String {
        guard snapshot.hasTrackers else {
...
private struct InlineTodayWidget: View {
    ...
    var body: some View {
        Text(snapshot.hasTrackers ? "Сегодня \(snapshot.completedCount)/\(snapshot.totalCount)" : "Сегодня нет привычек")
    }
}
...
private extension View {
    @ViewBuilder
    func trackerWidgetBackground() -> some View {
