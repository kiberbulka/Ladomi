...
final class StatisticViewController: UIViewController {
    ...
    private func dayWord(for count: Int) -> String {
        ...
        if remainder10 == 1 && remainder100 != 11 {
            return NSLocalizedString("dayItem.day", comment: "")
        } else if remainder10 >= 2 && remainder10 <= 4 && (remainder100 < 10 || remainder100 >= 20) {
            return NSLocalizedString("dayItem.2,3,4day", comment: "")
        } else {
            return NSLocalizedString("dayItem.days", comment: "")
        }
    }
...
extension StatisticViewController: DayItemStoreDelegate {
    func didUpdate(_ update: DayItemStoreUpdate) {
        reloadStatistics()
    }
}

extension StatisticViewController: DayItemRecordStoreDelegate {
